...
# per user model: 0.14148969349865204 -> 0.13901139194254009
explained_variance_score(result["rating"],
                         result["est_user"])

# per movie model: 0.22776182757516739 -> 0.22933166694190088
explained_variance_score(result["rating"],
                         result["est_movie"])

# avg of movie/user models: 0.26453133595803247 -> 0.2647144615259045
explained_variance_score(result["rating"],
                         (result["est_movie"] + result["est_user"]) / 2)

# weighted avg of movie/user models: 0.26915533848006079 -> 0.26897003092844141
...
# Export #
# ------ #
result.to_csv(r"data\validation_result\result_genre_prob.csv", index=False)
