...
MOVIE_FEATURES = ['year', 'rtAllCriticsRating', 'rtAudienceRating',
                  ...
                  'rtAllCriticsScore', 'rtAudienceScore', 'drama',
                  'comedy', 'crime', 'action', 'thriller', 'horror',
...
PER_MOVIE_KERNEL = GPy.kern.RBF(input_dim=len(USER_FEATURES),
                                variance=1.,
                                lengthscale=1.)
...
# per user model: 0.14148969349865204 -> 0.14078463974797195
explained_variance_score(result["rating"],
                         result["est_user"])

# per movie model: 0.22776182757516739 -> 0.23233456466089186
explained_variance_score(result["rating"],
                         result["est_movie"])

# avg of movie/user models: 0.26453133595803247 -> 0.26647984626585852
explained_variance_score(result["rating"],
                         (result["est_movie"] + result["est_user"]) / 2)

# weighted avg of movie/user models: 0.26915533848006079 -> 0.27069386761668845
...
# Export #
# ------ #
result.to_csv(r"data\validation_result\result_word2vec_and_genre_prob.csv",
              index=False)
